...
_RATE_LIMIT_DEFAULTS = {
    "development": {
        ...
        "RATE_LIMIT_ADMIN_LOGIN": "20/minute",
    },
    "testnet": {
...
_CURRENT_ADMIN_DEFAULTS = _ADMIN_DEFAULTS[ENVIRONMENT]
...
# Native transaction delivery is local transport scheduling, never consensus
# input. Defaults provide quick recovery without busy-looping an offline peer.
NATIVE_TRANSACTION_OUTBOX_POLL_INTERVAL_SECONDS = _env_int_any(
    ("NATIVE_TRANSACTION_OUTBOX_POLL_INTERVAL_SECONDS",), 1, minimum=1
)
NATIVE_TRANSACTION_OUTBOX_INITIAL_RETRY_SECONDS = _env_int_any(
    ("NATIVE_TRANSACTION_OUTBOX_INITIAL_RETRY_SECONDS",), 2, minimum=1
)
NATIVE_TRANSACTION_OUTBOX_MAX_RETRY_SECONDS = _env_int_any(
    ("NATIVE_TRANSACTION_OUTBOX_MAX_RETRY_SECONDS",), 300, minimum=1
)
NATIVE_TRANSACTION_OUTBOX_REQUEST_TIMEOUT_SECONDS = _env_int_any(
    ("NATIVE_TRANSACTION_OUTBOX_REQUEST_TIMEOUT_SECONDS",), 3, minimum=1
)
NATIVE_TRANSACTION_OUTBOX_LEASE_SECONDS = _env_int_any(
    ("NATIVE_TRANSACTION_OUTBOX_LEASE_SECONDS",), 30, minimum=1
)
NATIVE_TRANSACTION_RECONCILIATION_INTERVAL_SECONDS = _env_int_any(
    ("NATIVE_TRANSACTION_RECONCILIATION_INTERVAL_SECONDS",), 30, minimum=1
)
NATIVE_TRANSACTION_RECONCILIATION_BATCH_SIZE = _env_int_any(
    ("NATIVE_TRANSACTION_RECONCILIATION_BATCH_SIZE",), 100, minimum=1
)
if NATIVE_TRANSACTION_OUTBOX_INITIAL_RETRY_SECONDS > NATIVE_TRANSACTION_OUTBOX_MAX_RETRY_SECONDS:
    raise ValueError("NATIVE_TRANSACTION_OUTBOX_INITIAL_RETRY_SECONDS cannot exceed NATIVE_TRANSACTION_OUTBOX_MAX_RETRY_SECONDS.")
...
ADMIN_UI_ENABLED = _env_flag(
    "ADMIN_UI_ENABLED",
    _CURRENT_ADMIN_DEFAULTS["ADMIN_UI_ENABLED"],
)
ADMIN_AUTH_ENABLED = (
    _env_flag(
        "ADMIN_AUTH_ENABLED",
        _CURRENT_ADMIN_DEFAULTS["ADMIN_AUTH_ENABLED"],
    )
    if ENVIRONMENT == "development"
    else True
)
ADMIN_SESSION_TTL_SECONDS = _env_int_any(
    ("ADMIN_SESSION_TTL_SECONDS",),
    _CURRENT_ADMIN_DEFAULTS["ADMIN_SESSION_TTL_SECONDS"],
    minimum=60,
)
ADMIN_PASSWORD_HASH = _env_value_any(("ADMIN_PASSWORD_HASH",), "")
ADMIN_BOOTSTRAP_TOKEN = _env_value_any(("ADMIN_BOOTSTRAP_TOKEN",), "")
...
RATE_LIMIT_ADMIN_LOGIN = _env_value_any(
    ("RATE_LIMIT_ADMIN_LOGIN",),
    _CURRENT_RATE_LIMIT_DEFAULTS["RATE_LIMIT_ADMIN_LOGIN"],
)
...
RATE_LIMITS = {
    ...
    "admin_login": RATE_LIMIT_ADMIN_LOGIN,
}
...
PROTOCOL_V1_CONFIRMATION_DEPTH = _env_int_any(
    ("PROTOCOL_V1_CONFIRMATION_DEPTH", "CONFIRMATION_DEPTH"),
    2,
    minimum=0,
)
PROTOCOL_V1_FINALITY_DEPTH = _env_int_any(
    ("PROTOCOL_V1_FINALITY_DEPTH", "FINALITY_DEPTH"),
    6,
    minimum=0,
)
if PROTOCOL_V1_FINALITY_DEPTH < PROTOCOL_V1_CONFIRMATION_DEPTH:
    raise ValueError(
        "PROTOCOL_V1_FINALITY_DEPTH must be greater than or equal to "
