...
if (-not (Test-Path (Join-Path $ProjectRoot $env:SQLITE_DB_PATH))) {
    Write-Warning "No node-b SQLite database found. Run .\scripts\init_two_node_data.ps1 -Reset and migrate both node data directories before starting SQLite nodes."
}
