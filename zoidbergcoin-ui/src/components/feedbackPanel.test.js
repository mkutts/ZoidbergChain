...
test('blocked access gate exposes the in-app feedback entry point', () => {
  ...
  assert.match(source, /Choose how to connect your wallet/i);
  assert.match(source, /Continue with MetaMask/i);
  assert.match(source, /Continue with Email \/ Social Wallet/i);
  assert.match(source, /Use this if you already have a crypto wallet\./i);
  assert.match(source, /planned easier-onboarding path for new users, but it is still coming soon in this Vue app/i);
  ...
  assert.match(source, /selectedWalletProvider/i);
  ...
  assert.match(source, /portable wallet, export path, or migration path/i);
