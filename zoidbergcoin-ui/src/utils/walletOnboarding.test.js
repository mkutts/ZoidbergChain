...
test('wallet onboarding options separate live MetaMask access from future alternative login copy', () => {
  ...
  assert.equal(options[1].title, 'Email / Social Wallet');
  assert.equal(options[1].availability, 'coming_soon');
  assert.match(options[1].description, /simpler wallet option/i);
  assert.match(options[1].warning, /coming soon|live beta login/i);
});
